...
class GaussianDiffusion:
    ...
    def ddim_sample(
        ...
        if conf.inpa_inj_sched_prev:
            ...
            if pred_xstart is not None:
                ...
                #     plt.subplot(1,2,2)
                #     plt.imshow(gt_keep_mask.cpu().numpy()[0,0])
                #     plt.savefig('1.png')
        out = self.p_mean_variance(
            model,
            x,
    ...
    def ddim_sample_loop(
        ...
        final = None
        show_result={}
        for t, sample in self.ddim_sample_loop_progressive(
            model,
            shape,
            noise=noise,
            clip_denoised=clip_denoised,
            ...
            final = sample
            show_result[t] = np.copy(sample['sample'].cpu().numpy())
            # import ipdb;ipdb.set_trace()
        # import ipdb;ipdb.set_trace()
        sample['show']=show_result
        if return_all:
            return final
        else:
            return final["sample"]
    ...
    def ddim_sample_loop_progressive(
        ...
        if noise is not None:
            ...
            image_after_step = th.randn(*shape, device=device)
            image_after_step = self.ddim_reverse_sample_loop(
                model_instance, model_kwargs['gt'],
                clip_denoised=False,
                # device='cuda',
            )
        debug_steps = conf.pget('debug.num_timesteps')
        ...
        if conf.schedule_jump_params:
            ...
            for t_last, t_cur in time_pairs:
                ...
                #         plt.clf()
                #         plt.subplot(1,2,1)
                #         plt.imshow(image_before_step.cpu().numpy()[0,0])
                #         plt.subplot(1,2,2)
                #         plt.imshow(image_after_step.cpu().numpy()[0,0])
                #         plt.savefig('1.png')
